...
All compression methods here include the conversion from Float64 to Float32. The bilinear interpolation can impact the bitwise real
information content as defined in section \ref{sec:real_information}. A variable like cloud cover that takes only a few quantised states on
the original model grid will have more states when interpolated due to averaging. Consequently, the information in trailing
mantissa bits may be affected, but this is not further discussed here.
...
The compression methods described here are applied to gridded binary data. Data on structured grids can be represented as an array,
...
The conditional entropy (see section \ref{sec:cond_entropy})
	\begin{equation}
	H(s \vert r) = -\sum_{r=0}^1 \sum_{s=0}^1 p_{rs} \log_2 \left( \frac{p_{rs}}{p_r} \right)
	\end{equation}
is split into its contributions $H(s \vert r) = p_0H_0 + p_1H_1$. $H_0,H_1$ are the entropies conditioned on the state of the preceding bit
$b_{k-1}$ being $0$ or $1$, respectively
	\begin{align}
	H_0 &= -\frac{p_{00}}{p_0} \log_2(\frac{p_{00}}{p_0}) - \frac{p_{01}}{p_0} \log_2(\frac{p_{01}}{p_0}),\\
	H_1 &= -\frac{p_{10}}{p_1}  \log_2(\frac{p_{10}}{p_1} ) - \frac{p_{11}}{p_1}  \log_2(\frac{p_{11}}{p_1}). 
	\end{align}
...
dynamical system as predictands. The binwidth of the probability mass function is chosen to represent the uncertainty in the system, which the
bitwise real information strongly depends on. The formulation here avoids such an additional assumption of uncertainty as bits are used as both predictors and
...
In conclusion, while the information stored in temperature is independent of the unit, the required precision can change
and is in general application-dependent (see section \ref{sec:turing_test}).
...
\begin{figure}[tbhp]
	\includegraphics[width=1\textwidth]{Figures/compression/infograph_proof_cut.pdf}
	\caption{\textbf{The bitwise real information content is defined as the mutual information of bits in adjacent grid points.}
	A bit contains more real information the stronger the statistical dependence to the adjacent bits is. Independent
	bits contain only false information and are rounded to 0 to facilitate lossless compression.}
	\label{fig:infograph}
\end{figure}

We define the bitwise real information content as the mutual information
(\cite{Shannon1948,MacKay2003,Schreiber2000,Kraskov2004,Pothapakula2019,DelSole2004} and sections
\ref{sec:mutual_information}, \ref{sec:real_information}) of bits in
...
typical data requests from users. This problem is also faced by one-dimensional or without compression when the
data request does not match the data structure. Data from an array that is stored along longitudes in an archive cannot
be efficiently read when data in other dimensions is requested, e.g. a vertical profile at a single location. However,
multidimensional compression will inevitably exacerbate this problem and random access of large multidimensional data
sets, whether compressed or not, is an active field of research.
...
It is important to distinguish between the real bitwise information content as defined here and the required precision for a data set.
The required precision depends on the application: a budget analysis in a climate model likely requires a higher precision than a
highly uncertain variable in a weather forecast. In that sense, the real bitwise information content is one approach to quantify the
required precision, designed to be applicable in many use cases. However, as an information-based metric, solely derived from the
data itself, it cannot guarantee to estimate the required precision in all applications accurately. To further refine the required precision
for an application the iterative Turing test is proposed.
